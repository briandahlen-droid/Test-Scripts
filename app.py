...
def lookup_hillsborough_zoning_flu(address, folio=None, geometry=None):
    ...
    try:
        ...
        zoning_params = {
            'where': where_clause,
            'outFields': 'NZONE,NZONE_DESC,CATEGORY,FOLIO,FOLIONUM',
        ...
        if zoning_data.get('features'):
            ...
            st.write("DEBUG: Zoning query returned no features")
            st.warning("⚠️ No zoning data found for this folio. The zoning layer may use a different field name.")
        
        # Query FLU layer (still using coordinates if available, since FLU layer might not have FOLIO)
        # If we have geometry from parcel lookup, use it for FLU
        if geometry and geometry.get('rings'):
            ...
            st.write(f"DEBUG: Using parcel centroid for FLU: ({x}, {y})")
            ...
            st.write("DEBUG: No geometry available for FLU lookup")
        ...
        if county == "Pinellas":
            ...
            with st.spinner("Fetching property data from Hillsborough County..."):
                ...
                if result['success']:
                    ...
                    st.session_state['land_area_acres'] = result.get('site_area_acres', '')
                    st.session_state['api_flu'] = ''
                    st.session_state['hillsborough_folio'] = parcel_id_input  # Store folio for zoning lookup
                    st.session_state['hillsborough_geometry'] = result.get('geometry')  # Store geometry for FLU lookup
...
if st.button("🗺️ Lookup Zoning & Future Land Use", type="secondary"):
    ...
    if not address:
        ...
        if county == "Pinellas":
            ...
            # Get stored folio and geometry from parcel lookup
            folio = st.session_state.get('hillsborough_folio')
            geom = st.session_state.get('hillsborough_geometry')
            st.write(f"DEBUG: Folio = {folio}, Geometry available = {geom is not None}")
            
            with st.spinner(f"Fetching zoning/FLU data for {address}..."):
                zoning_result = lookup_hillsborough_zoning_flu(address, folio=folio, geometry=geom)
