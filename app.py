...
def lookup_hillsborough_zoning_flu(address, folio=None, geometry=None):
    ...
    try:
        ...
        # Query Hillsborough County ZONING (not land use, not FLU)
        # Layer 0 = Land Use, Layer 1 = Future Land Use, Layer 2 = ?
        st.write(f"DEBUG: Querying ZONING with coords ({x}, {y})")
        
        # Create a small envelope (bounding box) around the point
        buffer = 0.0001
        xmin, ymin = x - buffer, y - buffer
        xmax, ymax = x + buffer, y + buffer
        
        # Try layer 2 for actual zoning
        zoning_url = "https://maps.hillsboroughcounty.org/arcgis/rest/services/DSD_Viewer_Services/DSD_Viewer_Planning/MapServer/2/query"
        ...
        st.write(f"DEBUG: Zoning response: {zoning_data}")
        
        if zoning_data.get('features'):
            st.write(f"DEBUG: Zoning returned {len(zoning_data['features'])} features")
            attrs = zoning_data['features'][0]['attributes']
            st.write(f"DEBUG: Zoning attributes: {attrs}")
            
            # Try common zoning field names
            zoning_code = attrs.get('NZONE') or attrs.get('ZONING') or attrs.get('ZONE') or attrs.get('ZONECLASS') or ''
            zoning_desc = attrs.get('NZONE_DESC') or attrs.get('ZONING_DESC') or attrs.get('ZONE_DESC') or ''
            
            result['zoning_code'] = zoning_code
            result['zoning_description'] = zoning_desc
            st.write(f"DEBUG: Set zoning_code={zoning_code}, desc={zoning_desc}")
        else:
            st.write("DEBUG: Zoning query returned no features - trying different layer")
        ...
        if county == "Pinellas":
            ...
            with st.spinner("Fetching property data from Hillsborough County..."):
                ...
                if result['success']:
                    ...
                    st.session_state['land_area_acres'] = result.get('site_area_acres', '')
                    st.session_state['api_flu'] = ''
                    st.session_state['hillsborough_geometry'] = result.get('geometry')  # Store geometry for spatial query
