...
if st.button("Lookup Land Area", type="primary"):
    if not parcel_id:
        ...
        with st.spinner("Fetching land area from PCPAO..."):
            # Build URL matching the exact format from working browser URL
            # Note: For 200 CENTRAL AVE, browser shows s=173119731660010010
            # But removing dashes from 19-31-17-73166-001-0010 gives 193117731660010010
            # The strap transformation is unclear, using remove-dashes as best guess
            strap = parcel_id.replace('-', '')
            
            st.info(f"**Debug - Generated strap:** {strap}")
            
            # Use exact URL format that works in browser
            url = (
