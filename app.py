...
def lookup_hillsborough_zoning_flu(address, geometry=None):
    ...
    print(f"DEBUG: lookup_hillsborough_zoning_flu called with address: {address}")
    try:
        ...
        if geometry and geometry.get('rings'):
            ...
            print(f"DEBUG: Using parcel centroid: ({x}, {y})")
        else:
            # Geocode the address
            ...
            print(f"DEBUG: Geocode returned {len(geocode_data.get('candidates', []))} candidates")
        ...
        print(f"DEBUG: Tampa zoning response: {tampa_zoning_data}")
        
        # Debug: Check if Tampa query returned anything
        if 'error' in tampa_zoning_data:
            print(f"DEBUG: Tampa query had error: {tampa_zoning_data['error']}")
        elif tampa_zoning_data.get('features'):
            print(f"DEBUG: Tampa returned {len(tampa_zoning_data['features'])} features")
            ...
            print(f"DEBUG: Set zoning_code={result['zoning_code']}, desc={result['zoning_description']}")
        else:
            print(f"DEBUG: Tampa query returned no features")
        
        # If no Tampa zoning found, try Hillsborough County
        if not result.get('zoning_code'):
            print("DEBUG: Trying Hillsborough County zoning...")
            ...
            print(f"DEBUG: County zoning response: {county_zoning_data}")
            
            if county_zoning_data.get('features'):
                print(f"DEBUG: County returned {len(county_zoning_data['features'])} features")
                ...
                print("DEBUG: County query returned no features")
        
        print(f"DEBUG: Final result before FLU query: {result}")
