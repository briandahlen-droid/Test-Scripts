...
st.caption("Kimley-Horn - Tampa Bay Counties")

tab1, tab2, tab3, tab4, tab5 = st.tabs(["🏠 Property & Client", "📝 Project Understanding", "📋 Scope", "🔍 Permitting", "💰 Generate"])

with tab1:
    st.subheader("Property/Parcel Information")
    col1, col2 = st.columns(2)
    with col1:
        county = st.selectbox("County *", ["", "Pinellas", "Hillsborough", "Pasco"], key="county")
    with col2:
        parcel_id_input = st.text_input("Parcel ID *", placeholder="e.g., 19-31-17-73166-001-0010", key="parcel_input")
    ...
    with col1:
        ...
        st.text_input("Legal Entity Name *", key="legal_entity_name")
        st.text_input("Address Line 1 *", key="address_line1")
        st.text_input("Address Line 2 *", key="address_line2")
    with col2:
    ...
    with col3:
        st.date_input("Proposal Date *", value=date.today(), key="proposal_date")
        st.text_input("Project Name *", key="project_name")
    with col4:
        st.text_input("Project Address", value=st.session_state.get("api_address", ""), key="project_address")
        city_zip = f'{st.session_state.get("api_city", "")}, FL {st.session_state.get("api_zip", "")}'
        st.text_input("City, State, Zip", value=city_zip if st.session_state.get("api_city") else "", key="project_city_state_zip")
    st.text_area("Project Understanding *", height=200, key="project_description")

with tab2:
    st.info("Tab 2 - Project Understanding - To be built")

with tab3:
    st.info("Tab 3 - Scope - To be built")

with tab4:
    st.info("Tab 4 - Permitting - To be built")

with tab5:
    st.info("Tab 5 - Generate - To be built")
