    ...
    try:
        ...
        for folio_format in folio_formats:
            parcel_params = {
                ...
                'outFields': '*',
                'returnGeometry': 'true',
                'outSR': '4326',  # Request geometry in lat/lon!
                'f': 'json'
            }
...
def lookup_hillsborough_zoning_flu(address, geometry=None):
    ...
    st.write(f"DEBUG: lookup_hillsborough_zoning_flu called with address: {address}")
    try:
        ...
        # Try Tampa City zoning first (ArcGIS Online service)
        tampa_zoning_url = "https://services.arcgis.com/apTfC6SUmnNfnxuF/arcgis/rest/services/Zoning/FeatureServer/0/query"
        tampa_zoning_params = {
            'geometry': f'{x},{y}',
        ...
        st.write(f"DEBUG: Tampa zoning response: {tampa_zoning_data}")
        
        # Debug: Check if Tampa query returned anything
        if 'error' in tampa_zoning_data:
            st.write(f"DEBUG: Tampa query had error: {tampa_zoning_data['error']}")
        elif tampa_zoning_data.get('features'):
            st.write(f"DEBUG: Tampa returned {len(tampa_zoning_data['features'])} features")
            attrs = tampa_zoning_data['features'][0]['attributes']
            st.write(f"DEBUG: Tampa attributes: {attrs}")
            
            # Try multiple possible field name combinations
            result['zoning_code'] = attrs.get('ZONECLASS') or attrs.get('ZoneClass') or attrs.get('ZONE') or attrs.get('Zone') or ''
            result['zoning_description'] = attrs.get('ZONEDESC') or attrs.get('ZoneDesc') or attrs.get('Description') or ''
            result['source'] = 'Tampa City'
            st.write(f"DEBUG: Set zoning_code={result['zoning_code']}, desc={result['zoning_description']}")
        else:
            st.write(f"DEBUG: Tampa query returned no features")
        
        # If no Tampa zoning found, try Hillsborough County
        if not result.get('zoning_code'):
            st.write("DEBUG: Trying Hillsborough County zoning...")
            county_zoning_url = "https://services.arcgis.com/apTfC6SUmnNfnxuF/arcgis/rest/services/Zoning/FeatureServer/0/query"
            ...
            st.write(f"DEBUG: County zoning response: {county_zoning_data}")
            
            if county_zoning_data.get('features'):
                st.write(f"DEBUG: County returned {len(county_zoning_data['features'])} features")
                ...
                st.write("DEBUG: County query returned no features")
        
        st.write(f"DEBUG: Final result before FLU query: {result}")
        
        # Query FLU layer (county-wide, includes Tampa)
        st.write(f"DEBUG: Querying FLU with coords ({x}, {y})")
        ...
        st.write(f"DEBUG: FLU response: {flu_data}")
        ...
        if flu_data.get('features'):
            ...
            st.write(f"DEBUG: FLU set to: {result['future_land_use']}")
        else:
            st.write("DEBUG: FLU query returned no features")
        
        st.write(f"DEBUG: Final result being returned: {result}")
        ...
        if county == "Pinellas":
            ...
            with st.spinner("Fetching property data from Hillsborough County..."):
                ...
                if result['success']:
                    ...
                    st.session_state['land_area_acres'] = result.get('site_area_acres', '')
                    st.session_state['api_flu'] = ''
                    st.session_state['hillsborough_geometry'] = result.get('geometry')  # Store for zoning lookup
                    st.session_state['land_area_acres'] = result.get('site_area_acres', '')
                    st.session_state['api_flu'] = ''
...
if st.button("🗺️ Lookup Zoning & Future Land Use", type="secondary"):
    ...
    if not address:
        ...
        if county == "Pinellas":
            ...
            st.write("DEBUG: Hillsborough button branch triggered")
            st.write(f"DEBUG: Address = {address}")
            
            # Get stored geometry from parcel lookup
            geom = st.session_state.get('hillsborough_geometry')
            st.write(f"DEBUG: Geometry available = {geom is not None}")
            ...
            with st.spinner(f"Fetching zoning/FLU data for {address}..."):
                ...
                st.write("DEBUG - Function returned:", zoning_result)
                
                if zoning_result.get('success'):
                    # Update zoning - check multiple ways
                    zoning_code = zoning_result.get('zoning_code', '')
                    zoning_desc = zoning_result.get('zoning_description', '')
                    ...
                    if zoning_code:
                        ...
                        st.info(f"✅ Zoning set to: {st.session_state['api_zoning']}")
                    else:
                        st.warning("⚠️ No zoning code returned from API")
                    
                    # Update FLU
                    flu_value = zoning_result.get('future_land_use', '')
                    if flu_value:
                        st.session_state['api_flu'] = flu_value
                        st.info(f"✅ FLU set to: {st.session_state['api_flu']}")
                    else:
                        st.warning("⚠️ No FLU returned from API")
                    
                    st.success(f"✅ Lookup completed!")
                    # st.rerun()  # Temporarily disabled to see debug messages
                else:
                    st.error(f"❌ {zoning_result.get('error', 'Unable to fetch zoning data')}")
...
if st.session_state.get('api_city'):
    ...
    if st.session_state.get('api_flu'):
        ...
        if county == "Pinellas":
            st.info("ℹ️ Click '🗺️ Lookup Zoning & Future Land Use' button to get detailed zoning data")
        else:
            st.info("ℹ️ Click '🗺️ Lookup Zoning & Future Land Use' button to get detailed zoning data for Hillsborough County")
else:
