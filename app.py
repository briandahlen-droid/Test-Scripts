        ...
        if not is_valid:
            ...
            with st.spinner("Fetching property data from PCPAO API..."):
                ...
                if result['success']:
                    ...
                    # For St. Petersburg, lookup zoning from GIS layers
                    city = result.get('city', '')
                    address = result.get('address', '')
                    
                    if 'St. Petersburg' in city and address:
                        with st.spinner("Looking up zoning from St. Pete GIS layers..."):
                            zoning_result = lookup_pinellas_zoning(city, address)
                            
                            if zoning_result['success']:
                                # Store zoning info
                                zoning_display = zoning_result.get('zoning_code', '')
                                if zoning_result.get('zoning_description'):
                                    zoning_display = f"{zoning_display} - {zoning_result['zoning_description']}"
                                st.session_state['api_zoning'] = zoning_display
                                
                                # Store FLU info
                                flu_display = zoning_result.get('future_land_use', '')
                                if zoning_result.get('future_land_use_description'):
                                    flu_display = f"{flu_display} - {zoning_result['future_land_use_description']}"
                                st.session_state['api_flu'] = flu_display
                            else:
                                st.session_state['api_zoning'] = 'Could not lookup zoning from GIS layers'
                                st.session_state['api_flu'] = ''
                    else:
                        # Non-St. Pete cities
                        st.session_state['api_zoning'] = result.get('zoning', 'Contact City/County for zoning info')
                        st.session_state['api_flu'] = ''
...
if st.session_state.get('api_city'):
    ...
    # Show zoning lookup status
    if st.session_state.get('api_zoning') and 'Could not lookup' not in st.session_state.get('api_zoning', ''):
        st.success("✅ St. Pete GIS Layer Lookup completed successfully")
        zoning_retrieved = []
        if st.session_state.get('api_zoning'): zoning_retrieved.append("Zoning")
        if st.session_state.get('api_flu'): zoning_retrieved.append("Future Land Use")
        st.info(f"**Retrieved from GIS Layers:** {', '.join(zoning_retrieved)}")
else:
    st.info("Click 'Lookup Property Info' to test PCPAO API + GIS layer lookups")
