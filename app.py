...
def lookup_pasco_parcel(parcel_id):
    ...
    st.write(f"DEBUG: lookup_pasco_parcel called with: {parcel_id}")
    try:
        ...
        st.write(f"DEBUG: Trying formats: {parcel_formats}")
        ...
        for parcel_format in parcel_formats:
            ...
            st.write(f"DEBUG: Querying with PARCELID = '{parcel_format}'")
            parcel_resp = requests.get(parcel_url, params=parcel_params, timeout=15)
            data = parcel_resp.json()
            st.write(f"DEBUG: Response features count: {len(data.get('features', []))}")
            if data.get('features'):
                parcel_data = data
                st.write(f"DEBUG: Found match with format: {parcel_format}")
                break
        ...
        st.write("DEBUG - Pasco SWFWMD Response Fields:")
        st.write(f"SITEADD: {attrs.get('SITEADD')}")
...
st.title("🏠 Florida Property Lookup - Pinellas & Hillsborough Counties")
st.caption("Multi-county property data: City, Address, Owner, Property Use, Zoning, Future Land Use, Land Area")
st.markdown("---")
