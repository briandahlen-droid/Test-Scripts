...
def lookup_hillsborough_zoning_flu(address, folio=None, geometry=None):
    ...
    try:
        ...
        # Query Hillsborough County zoning using SPATIAL INTERSECTION with ENVELOPE
        st.write(f"DEBUG: Querying zoning with coords ({x}, {y})")
        
        # Create a small envelope (bounding box) around the point
        # Buffer of ~0.0001 degrees (about 10 meters)
        buffer = 0.0001
        ...
        # Use envelope geometry instead of point
        zoning_params = {
            'where': '1=1',
            'geometry': f'{xmin},{ymin},{xmax},{ymax}',  # Envelope format
            'geometryType': 'esriGeometryEnvelope',  # Changed to envelope
